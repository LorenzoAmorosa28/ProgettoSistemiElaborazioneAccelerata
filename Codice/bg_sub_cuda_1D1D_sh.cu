...
__global__ void bgSubGaussian(
    ...
    //5 righe × (BLOCK_SIZE + 4 colonne)
    __shared__ unsigned char tile[5][BLOCK_SIZE + 4];
    ...
    for (int dy = -2; dy <= 2; dy++) {
        ...
        //pixel centrale
        int xx = min(max(baseX + tx, 0), cols - 1);
        tile[dy + 2][tx + 2] = frame[yy * cols + xx];

        //sinistra
        if (tx < 2) {
            int xl = min(max(baseX + tx - 2, 0), cols - 1);
            tile[dy + 2][tx] = frame[yy * cols + xl];
        }

        //destra
        if (tx >= blockDim.x - 2) {
            int xr = min(max(baseX + tx + 2, 0), cols - 1);
            tile[dy + 2][tx + 4] = frame[yy * cols + xr];
        }
    }

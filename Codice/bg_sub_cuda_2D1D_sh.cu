...
__global__ void bgSubGaussian(
    ...
    __shared__ unsigned char tile[5][256+4];

    const int R = 2;
    const int tileW = blockDim.x + 2 * R;   // 260
    const int tileH = 1 + 2 * R;            // 5
    ...
    // Ogni thread carica la colonna 
    for (int dy = -R; dy <= R; dy++) {
        int yy = min(max(y + dy, 0), rows - 1);
        int tileY = dy + R;

        int xx = min(max(x, 0), cols - 1);

        tile[tileY][tx] = frame[yy * cols + xx];
    }
    ...
    if (threadIdx.x < R) {
        for (int dy = -R; dy <= R; dy++) {
            ...
            int xx = min(max(x - R, 0), cols - 1);

            tile[tileY][threadIdx.x] = frame[yy * cols + xx];
        }
    }
    ...
    if (threadIdx.x >= blockDim.x - R) {
        for (int dy = -R; dy <= R; dy++) {
            ...
            int tileX = tx + R;
            tile[tileY][tileX] = frame[yy * cols + xx];
        }
    }
    ...
    for (int dy = -2; dy <= 2; dy++) {
        for (int dx = -2; dx <= 2; dx++) {
            ...
            int tileX = tx + dx;
            int tileY = dy + R;
            sum += kernel[dy + 2][dx + 2] * tile[tileY][tileX];
        }
    }
...
int main(int argc, char** argv) {
    ...
    dim3 block(256, 1, 1);
    dim3 grid(
    (cols + 256 - 1) / 256, // blocchi per riga
    rows,                           
    1
    );

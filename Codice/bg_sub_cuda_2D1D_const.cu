...
//dichiarato in memoria costante
__constant__ int d_gaussianKernel[25];
...
__global__ void bgSubGaussian(
    ...
    for (int dy = -2; dy <= 2; dy++) {
        ...
        for (int dx = -2; dx <= 2; dx++) {
            ...
            int k = d_gaussianKernel[(dy + 2) * 5 + (dx + 2)];

            sum += k * frame[yy * cols + xx];
        }
    }

    float pixel = sum * (1.0f / 256.0f);
